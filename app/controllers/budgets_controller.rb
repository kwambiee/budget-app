class BudgetsController < ApplicationController
  def index
    ...
    @budgets = CategoryBudget.includes(:category, :budget).where(category: @category).order('budgets.created_at DESC').pluck(
      'budgets.name', 'budgets.amount', 'budgets.created_at'
    )
    @total_amount = @budgets.sum { |_string, number, _date| number }
  end
